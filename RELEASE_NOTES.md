# Swordigo v1.0.14 — PortMaster dialog handoff

Candidate ZIP SHA-256:
`81c13ed64529c755407988cdc008622fda2dcd461fcbc8e33704c715f5524717`

This launcher-only maintenance candidate is generated by immutable
`nxbootstrap-v0.6.7` at framework commit `3ad5d94`. It closes the live
PortMaster dialog through `PortMasterDialogExit` before NXExtract or the game
starts, verifies that `PM_PIPE` was cleared, and still calls `pm_finish`
exactly once on exit. The cumulative stat-free lock and early per-process
diagnostic remain in place.

The approved Swordigo loader is byte-identical to v1.0.13. Native lifecycle,
video, audio, controls and owner data are unchanged. The exact ZIP above was
physically clean-extracted from the APK on an ArkOS RK3326/Mali-G31 device with
a deliberately failing `stat` in `PATH`: NXExtract committed 941 assets and 9
music files transactionally, the game reached a non-black 640×480 frame with
OpenAL/music, and shutdown completed with status 0.

## Português

Esta candidata de manutenção altera somente o launcher, gerado pelo
`nxbootstrap-v0.6.7` imutável no commit `3ad5d94` do framework. Antes do
NXExtract ou do jogo, ele fecha o diálogo vivo do PortMaster pela API
`PortMasterDialogExit`, confirma que `PM_PIPE` foi limpo e mantém uma única
chamada de `pm_finish` na saída. O lock cumulativo sem `stat` e o diagnóstico
inicial por processo continuam presentes.

O loader aprovado é byte a byte idêntico ao da v1.0.13. Ciclo nativo, vídeo,
áudio, controles e dados do usuário não mudaram. O ZIP exato acima foi testado
fisicamente num ArkOS RK3326/Mali-G31 com um `stat` propositalmente inválido no
`PATH`: o NXExtract publicou transacionalmente 941 assets e 9 músicas, o jogo
chegou a um quadro 640×480 não preto com OpenAL/música e encerrou com status 0.

---

# Swordigo v1.0.12 — AmberELEC portable instance lock

Candidate ZIP SHA-256:
`a3e62c6088c44f885de65f30adbda94bddbfdcad36f1d2b8a19d58cdc7871c48`

This maintenance candidate updates the generated launcher to nxbootstrap
0.6.2. AmberELEC builds BusyBox `stat` without custom `-c` formats, so the
0.6.1 identity probe returned empty values and rejected a valid lock before the
game started. The new probe uses Bash `-ef` for path-to-fd identity and portable
`stat -t` for the hardlink count. Symlink, ownership, hardlink, double-launch,
atomic-replacement and leaked-fd protections remain fail-closed.

Swordigo's loader, native lifecycle, video, audio, controls, extraction and
owner data are unchanged from 1.0.11. Host/build/package gates cover the fix;
physical AmberELEC gameplay of the exact release ZIP remains pending.

## Português

Esta candidata de manutenção atualiza o launcher gerado para o nxbootstrap
0.6.2. O AmberELEC compila o `stat` do BusyBox sem os formatos personalizados
de `-c`; por isso o probe 0.6.1 recebia valores vazios e recusava um lock válido
antes de iniciar o jogo. O novo probe usa `-ef` do Bash para comparar caminho e
descritor e `stat -t` para contar hardlinks. As proteções contra symlink,
hardlink, duas instâncias, troca atômica e vazamento de descritor continuam
fail-closed.

Loader, ciclo nativo, vídeo, áudio, controles, extração e dados do usuário não
mudaram desde a 1.0.11. A correção passa pelos gates de host/build/pacote; o
gameplay físico do ZIP exato no AmberELEC ainda está pendente.
