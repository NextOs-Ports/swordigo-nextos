...
Candidate ZIP SHA-256: pending the final exact-ZIP gate.
...
It also refuses phantom axes from the mapping. On the tested controller SDL's
database says `rightx:a2,righty:a3`, while the opened joystick has only two raw
axes. The loader now reports one real stick and enables a capability-only
bridge: hold L2 and move the left stick for the cursor; R2/R3 clicks or drags.
Without L2 the left stick keeps native movement, and D-pad/face buttons are
never repurposed.
...
PortMaster dialog handoff and exactly one `pm_finish`. Final publication waits
for clean NXExtract and manual controls/cursor acceptance of the exact ZIP.
...
O mapping também não pode mais inventar eixos. No controle testado o banco da
SDL declara `rightx:a2,righty:a3`, mas o joystick aberto possui só dois eixos
crus. O loader reconhece um único analógico e libera um bridge por capacidade:
segure L2 e mova o esquerdo para controlar a seta; R2/R3 clica ou arrasta. Sem
L2, o esquerdo continua movimentando o personagem; D-pad e botões de face nunca
são roubados.
...
diálogo do PortMaster e uma única chamada de `pm_finish`. A publicação final
aguarda NXExtract limpo e aceitação manual de controles/cursor no ZIP exato.
