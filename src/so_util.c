...
/* Manual mmap modules are invisible to libc dl_iterate_phdr; the statically
 * linked unwinder inside libswordigo.so needs PT_GNU_EH_FRAME via this list. */
#define SO_MAX_PH 20
struct so_phdr_mod g_so_mods[4];
int g_so_nmods = 0;

void so_record_phdr(const char *name) {
  if (g_so_nmods >= 4 || !elf_hdr || !prog_hdr || !load_virtbase)
    return;
  struct so_phdr_mod *m = &g_so_mods[g_so_nmods++];
  m->base = (uintptr_t)load_virtbase;
  int n = elf_hdr->e_phnum;
  if (n > SO_MAX_PH)
    n = SO_MAX_PH;
  m->phnum = n;
  for (int i = 0; i < n; i++)
  ...
  debugPrintf("so_record_phdr: %s base=%p phnum=%d\n", m->name,
              (void *)m->base, m->phnum);
}
